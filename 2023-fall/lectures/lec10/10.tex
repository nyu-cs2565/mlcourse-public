...
\begin{itemize}
    \item<+-> The reversed order: The last dimention ($D_4$) is preserved throughout propagation.
    \item<+-> The forward order: The first dimension ($D_1$) is preserved throughout propagation.
...
\begin{itemize}
    \item The typical image input layer has 3 channels R G B for color or 1 channel for grayscale.
...
\includegraphics<1>[width=0.5\textwidth]{figures/stride2_conv/frame_0.png}
\includegraphics<2>[width=0.5\textwidth]{figures/stride2_conv/frame_1.png}
\includegraphics<3>[width=0.5\textwidth]{figures/stride2_conv/frame_2.png}
\includegraphics<4>[width=0.5\textwidth]{figures/stride2_conv/frame_3.png}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    ...
    \item We can consider the distribution of whole dataset in the activation space.
...
\begin{itemize}
    ...
    \pause
    \item To make the variance constant, we need $\frac{1}{2} n_l Var[w_{l}] = 1$, $Std[w_{l}] = \sqrt{2/n_l}$.
\end{itemize}
    See original paper\footnote{He et al. Delving Deep into Rectifiers: Surpassing Human-Level Performance on ImageNet. ICCV, 2015.}.
\end{frame}

\begin{frame}{Activation functions}
\begin{itemize}
    \item ReLU was proposed in 2009-2010\footnote{Jarrett et al. What is the Best Multi-Stage Architecture for Object Recognition? ICCV, 2009.}\footnote{Nair \& Hinton/ Rectified Linear Units Improve Restricted Boltzmann Machines. ICML, 2010.}, and was successfully used in AlexNet for image classification in 2012\footnote{Krizhevsky et al. 
...
\begin{frame}{Adam}
\begin{itemize}
  \item \alert{Adam} = RMSprop + momentum
\end{itemize}
\end{frame}
...
\begin{frame}{Residual connection}
\end{frame}
...
\begin{frame}{Recurrent networks}
\end{frame}
