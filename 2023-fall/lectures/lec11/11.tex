...
\includegraphics[width=0.7\textwidth]{figures/rnn.png}
\end{center}
Image credit: Chris Olah~\footnote{https://colah.github.io/posts/2015-08-Understanding-LSTMs/}
\end{frame}
...
\begin{itemize}
    ...
    \item In turns out that just attention is sufficient, with additional ``positional encoding''
\end{itemize}
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/transformer_attention.png}\\
Image credit: \footnote{https://blog.research.google/2017/08/transformer-novel-neural-network.html}
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \item It's found to be more advantageous to have multiple set of attentions for each token, so it can more efficiently incorporate information from multiple sources.
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\begin{center}
...
\begin{frame}
\begin{itemize}
    \item Attention is expensive - quadratic to the number of tokens
    \item 
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \[
    p(x_{1:t}) = p(x_1, \dots, x_t) = p(x_1) p(x_2 | x_1) \dots p(x_t | x_{t-1})
    \]
\end{itemize}
\end{frame}
