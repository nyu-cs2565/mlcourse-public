...
$$f(x') \ge f(x) + \nabla f(x) \dot (x' - x) + \frac{\mu}{2} \lVert x - x' \rVert^2$$
\end{definition}
\begin{center}
...
\pause
\item If $\eta_k = \eta / k$, then $\sum_k \eta_k = O(log(t)) \sum_k \eta_k^2 = O(1)$, error=$\textcolor{red}{O(1/\log(t))}$.
\pause
...
\begin{itemize}
    \item Outlier is a data point that differs significantly from other observations.
    \pause
