...
\begin{itemize}
\item SVM objective function (letting $\lambda = \frac{1}{2c}$):
\begin{align*}
    J(w)&=\frac{c}{n}\sum_{i=1}^{n}\max\left(0,1-{y_{i}w^{T}x_{i}}\right)+\frac{1}{2}||w||^{2}.\\
   \Rightarrow J(w)&=\frac{1}{n}\sum_{i=1}^{n}\max\left(0,1-{y_{i}w^{T}x_{i}}\right)+\lambda||w||^{2}.
\end{align*}
\pause
...
\]
    ...
    \end{figure}
    What subgradient is the Pegasos algorithm taking? 
\end{frame}
