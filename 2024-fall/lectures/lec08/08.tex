...
\begin{frame}{Structured SVM inference (linear chain)}
% Dynamic programming (similar to the Viterbi algorithm)
\begin{center}
\includegraphics[height=0.35\textheight]{figures/dp}
\end{center}
\begin{itemize}
  \item Initiate $\alpha_j (1) = w^\top \psi(y_1=j, x_1)$
  \pause
  \item Recursion $\alpha_j (t) = \max_i \alpha_i (t-1) + w^\top \psi(y_t=j, y_{t-1}=i, x_t)$
  \pause
  \item Pointer $\gamma(t, j) = \argmax_i \alpha_i (t-1) + w^\top \psi(y_t=j, y_{t-1}=i, x_t)$
  \pause
  \item Backtrack: $r(T) = \argmax_i \alpha_i(T), r(t) = \gamma(t, r(t+1))$
\end{itemize}
\pause
What's the running time?
\note<.>{Let $K=\vert\sY\vert$, DP runtime $O(K^2L)$, $m$th order Markov feature has runtime $O(K^mL)$, naive runtime $O(K^L)$.}
...
\begin{itemize}
  ...
  \item $p(y|x) = \frac{1}{Z(x)} \exp(w^\top \psi(x, y))$.
  \item Loss function:
  \begin{align*}
...
\end{itemize}
\pause
\item Structured prediction: Structured SVM, CRF. Data containing structure. Extremely large output space. Text and image applications. More in-depth content in a probabilistic graphical model (PGM) course.
\end{itemize}
\end{frame}
