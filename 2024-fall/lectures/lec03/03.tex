...
\title[CSCI-GA 2565]{Controling Complexity: Regularization}
\author[]{Mengye Ren
\\ \quad\\
...
% \begin{itemize}
%     \item For those of you who want to take notes on your tablets.
%     \item Otherwise, slides will be shared on the course website after the lecture.
% \end{itemize}
% \begin{center}
% \includegraphics[width=0.35\textwidth]{figures/qr_code3.png}
% \end{center}
% \end{frame}
...
\begin{eqnarray*}
\ell(f(x),y) & = & \left(f(x)-y\right)^{2} \\ \pause
& = & f^2(x) - 2f(x)y + y^2 \\ \pause
& = & f^2(x) y^2 - 2f(x)y + 1 \\ \pause
& = & \left(1-f(x)y\right)^{2}\\ \pause
& = & \left(1-m\right)^{2}
\end{eqnarray*}
\end{itemize}
\end{frame}
...
\section{Controlling the Complexity through Regularization}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item Could there be a more consistent way of formulating feature selection as an optimization problem?
    \end{itemize}
\end{frame}
...
\begin{frame}{Soft Selection}
    \begin{itemize}
        \item We can imagine having a weight for each feature dimension.
        \pause
        \item In linear regression, the model weights multiply each feature dimension:
        $$f(x) = w^\top x$$
        \pause
        \item If $w_i$ is zero or close to zero, then it means that we are not using the $i$-th feature.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item More stable: small change in the input does not cause large change in the output
...
\begin{frame}
    ...
    \begin{figure}
        \includegraphics[height=4cm]{figures/poly-regression}
    \end{figure}
    \begin{itemize}
    \vspace{-0.5cm}
    \item n-th feature dimension is the n-th power of x: $1, x, x^2, ...$ 
    \pause
...
\begin{frame}
    ...
    \begin{itemize}
        \setlength\itemsep{1ex}
        \item Linear: $X^TXw = X^Ty$ -> $w = (X^T X)^{-1} X^T y$
        \pause
        \item Ridge: $(X^TX + {\color{blue}\lambda I})w = X^Ty$ -> $w = (X^T X + {\color{blue}\lambda I})^{-1} X^T y$
            \begin{itemize}
...
\begin{itemize}
    \item L2 regularizer is a term in our optimization objective.
    $$w^* = \argmin_{w} \frac{1}{2}\|Xw - y\|_2^2 + {\color{blue}\frac{\lambda}{2}{\|w\|_2^2}}$$
    \item This is also called the \textbf{Tikhonov} form.
    \pause
    \item The Lagrangian theory allows us to interpret the second term as a constraint.
    $$w^* = \argmin_{w: ||w||^2_2 \le r} \frac{1}{2}\|Xw - y\|_2^2$$
    \item At optimum, the gradients of the main objective and the constraint cancel out.
    % \item The L2 norm needs to be less than $r$, for some $\lambda$.
    \item This is also called the \textbf{Ivanov} form.
\end{itemize}
\end{frame}
...
% \end{itemize}
% \pause
% \item The conditions for this equivalence can be derived from the Lagrangian theory.
% \pause
...
\let\thefootnote\relax\footnotetext{\tiny{Modified from Hastie, Tibshirani, and Wainwright's \emph{Statistical Learning with Sparsity}, Fig 2.1. About predicting crime in 50 US cities.}}
\begin{center}
\includegraphics[height=0.6\textheight]{figures/lasso-ridge-paths-side-by-side}
\par\end{center}
...
\begin{frame}{Regularization as Constrained ERM}
...
% \begin{frame}{Second Option: Projected SGD}
% \begin{itemize}
...
% \begin{frame}{Third Option: Coordinate Descent Method}
...
% \item $w^{(t+1)}\gets w^{(t)}$ and $w_{j}^{(t+1)}\gets w_{j}^{\mbox{new}}$
% \item $t\gets t+1$
% \end{itemize}
% \end{itemize}
% \end{block}
...
% Set the gradient of $w_j$ to 0. Let $w_{-j}$ denote $w$ without the $j$-th component, and $x_{i,-j}$ denote $x_i$ without the $j$-th component.
% \pause
% \[
% 2\sum_i (w^T x_i - y_i) x_{i,j} + \lambda \frac{|\hat{w}_j|}{\hat{w}_j} = 0\]
% \pause
% \[
% 2\sum_i (\hat{w}_j x_{i,j} + w_{-j}^T x_{i,-j} - y_i) x_{i,j} + \lambda \frac{|\hat{w}_j|}{\hat{w}_j} = 0 \]
% \pause
% \[\hat{w}_j 2 \sum_i x_{i,j}^2 + 2\sum_i (w_{-j}^T x_{i,-j} - y_i) x_{i,j} + \lambda \frac{|\hat{w}_j|}{\hat{w}_j} = 0 \]
% \end{frame}
...
% \[
% \hat{w}_j \textcolor{red}{\underbrace{2 \sum_i x_{i,j}^2}_{a_j}} - \textcolor{blue}{\underbrace{2 \sum_i (y_i - w_{-j}^T x_{i,-j} ) x_{i,j}}_{c_j}} + \lambda \frac{|\hat{w}_j|}{\hat{w}_j} = 0 \]
% \pause
% \[
% \hat{w}_j \textcolor{red}{a_j} - \textcolor{blue}{c_j} + \lambda \text{sgn}(\hat{w_j}) = 0 \]
% \pause
% \[
% \hat{w}_{j}=
% \begin{cases}
% \frac{c_j-\lambda}{a_j} & \mbox{if } \hat{w}_j > 0 \\
% \frac{c_j+\lambda}{a_j} & \mbox{if } \hat{w}_j < 0 \\
% [c_j - \lambda, c_j + \lambda]  & \mbox{if } \hat{w}_j = 0
% \end{cases}
% \]
% \end{frame}
...
% \]
% \pause
% Because $a_j = \sum_i x^2_{i,j} \ge 0$, so 
% \begin{align*}
% \hat{w}_j=
% \begin{cases}
% \frac{c_j-\lambda}{a_j} & \mbox{if } c_j-\lambda > 0 \\
% \frac{c_j+\lambda}{a_j} & \mbox{if } c_j+\lambda < 0 \\
% 0  & \mbox{if } -\lambda \le c_j \le \lambda
% \end{cases}
% \end{align*}

% The lasso objective coordinate minimization has a closed form.
% \end{frame}
...
% \begin{frame}{Summary}
% \begin{itemize}
%     \item Controlling the complexity of the hypothesis space
%     \pause
%     \item Feature selection
%     \pause
%     \item Regularization
%     \pause
%     \item L2 vs. L1 regularization (ridge and lasso)
%     \pause
%     \item Tikhonov vs. Ivanov (soft penalty vs. hard constraint)
%     \pause
%     \item Three ways of optimizing lasso regression: QP, Project SGD, Coordinate Descent
% \end{itemize}
% \end{frame}
