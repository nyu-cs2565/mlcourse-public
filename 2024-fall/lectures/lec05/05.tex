...
\begin{frame}{Slides}
\begin{center}
\includegraphics[width=0.3\textwidth]{figures/qrcode_05.png}
\end{center}
\end{frame}

\begin{frame}{Logistics}
\begin{itemize}
    \item Today (Oct 1): Kernels and Probabilistic Modeling
    \item Oct 8: Guest Lecture
    \item Oct 15: Homework 2 Due
    \item Oct 15: Legislative Day No Class
    \item Oct 22: Midterm, in class, closed-book, covers everything including Oct 8
\end{itemize}
\end{frame}
...
\end{frame}
%
\begin{frame}{Compute the Inner Products}
    ...
    \mode<handout>{
    \begin{eqnarray*}
    \psi(x)^T\psi(x') & = & 1 + 2x_1x'_1 + 2 x_2 x'_2 + x_1^2 x^{\prime 2}_1 + 2 x_1 x_2 x'_1 x'_2 + x_2^2 x^{\prime 2}_2 \\
    & = & 1 + 2 (x_1 x'_1 + x_2 x'_2) + (x_1 x'_1 + x_2 x'_2)^2 \\
    & = & (1 + x^\top x')^2
    \end{eqnarray*}
    }
    \pause
...
\item \textbf{Feature space}: $\ch$ 
% (a Hilbert space, e.g. $\reals^{d}$)
...
\mode<handout>{
\begin{frame}{Quadratic Kernel}
\begin{eqnarray*}
k(x,x') & = & \left\langle \psi(x),\psi(x')\right\rangle \\
& = & \sum_{ij} x_i x_j' + \sum_{ij} x_i^2 x_j'^2 + 2 \sum_{ijkl} x_i x_j x'_k x'_l \\
& = & \sum_{ij} x_i x_j' + \left(\sum_{ij} x_i x_j'\right)^2 \\
& = & \left\langle x,x'\right\rangle +\left\langle x,x'\right\rangle ^{2}
% & = & \left\langle x,x'\right\rangle  + \sum_{ij} x_i^2 x_j'^2 + \sum_{ijkl} 2 x_i x_j x'_k x'_l
% \left\langle x,x'\right\rangle +\left\langle x,x'\right\rangle ^{2}
\end{eqnarray*}
\end{frame}
}
...
Ridge regression solution: $w^{*}=\left(X^{T}X+\lambda I\right)^{-1}X^{T}y$.\\
Lemma: If $A$ and $A+B$ are non-singular, then $(A+B)^{-1} = A^{-1} - A^{-1} B (A+B)^{-1}.$

Let $\lambda I = A$, $X^T X = B$,
\begin{eqnarray*}
w^{*} & = & \left(X^{T}X+\lambda I\right)^{-1}X^{T}y\\
& = & (\lambda^{-1} - \lambda^{-1} X^TX (X^TX + \lambda I)^{-1}) X^T y \\
& = & X^T \lambda^{-1} y - \lambda^{-1} X^TX (X^TX + \lambda I)^{-1} X^T y \\
& = & X^T \lambda^{-1} y - \lambda^{-1} X^TX w^* \\
& = & X^{T}\underbrace{\left(\frac{1}{\lambda}y-\frac{1}{\lambda}Xw^{*}\right)}_{\alpha^{*}}\\
 & = & X^{T}\alpha^{*}=\sum_{i=1}^{n}\alpha_{i}^{*}x_{i}.
\end{eqnarray*}
\end{frame}
}
...
\mode<handout>{
\begin{frame}{Matrix Sum Inverse Lemma Derivation}
Woodbury identity: $(A + UCV)^{-1} = A^{-1} - A^{-1} U (C^{-1} + V A^{-1} U)^{-1} V A^{-1}$

Let $C, V = I$, $U = B$,
\begin{eqnarray*}
(A + B)^{-1} & = & A^{-1} - A^{-1} B (I + A^{-1} B)^{-1} A^{-1} \\
& = & A^{-1} - A^{-1} B (A (I + A^{-1} B))^{-1} \\
& = & A^{-1} - A^{-1} B (A + B)^{-1}.
\end{eqnarray*}
\end{frame}
}
...
\section{Probabilistic Modeling: Overview}
\begin{frame}
...
\mode<presentation>{\vspace{2.0in}}
...
\ell(\theta) &= \sum_{n=1}^N \log p(y^{(n)} \mid x^{(n)}; \theta) \\
&= \sum_{n=1}^N y^{(n)}\log f(\theta^T x^{(n)}) + (1-y^{(n)})\log (1 - f(\theta^T x^{(n)}))
\end{align}
}
...
\begin{frame}{Example: multinomial logistic regression}
How to extend logistic regression to multiclass classification?
\pause
Output: Bernoulli distribution $\rightarrow$ \textcolor{blue}{categorical distribution}
\begin{itemize}
...
\item What's the transfer function that maps this $\BR^{k}$ vector into a probability?
...
\mode<presentation>{
\textcolor{green}{Exercise}:} Set $\frac{\partial}{\partial \theta_{j,1}}\ell$ to zero, show that
$
...
\begin{itemize}
% \item What if input dimensions are not independent?
...
\end{figure}
\onslide<2->\item Mahalanobis distance $(\bx - \mu_k)^T \Sigma^{-1} (\bx - \mu_k)$ measures the distance from $\bx$ to $\mu$.
...
\[
p(\bx|y=k) = \frac{1}{(2 \pi)^{d/2} |\Sigma_k|^{1/2}} \exp \left[ -\frac{1}{2} (\bx - \boldsymbol{\mu}_k)^T \Sigma_k^{-1} (\bx - \boldsymbol\mu_k) \right] \
\]
...
\vspace{-0.3cm}
\begin{figure}
    \includegraphics[width=0.9\linewidth,trim={5cm 1cm 5cm 1cm},clip]{figures/gbc_example}
\end{figure}
\end{frame}
